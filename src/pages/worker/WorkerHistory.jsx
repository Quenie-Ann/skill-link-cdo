...
import {
  ...
  Sun, Moon, CheckCircle2, Clock, XCircle,
  TrendingUp, Filter, Search, Star
} from 'lucide-react';
...
export default function WorkerHistory() {
  ...
  const filtered = JOB_HISTORY.filter((job) => {
    const matchSearch =
  ...
  const totalEarned    = JOB_HISTORY.filter((j) => j.status === 'completed').reduce((a, j) => a + j.pay, 0);
  const completedCount = JOB_HISTORY.filter((j) => j.status === 'completed').length;
  const avgRating      = (
    JOB_HISTORY.filter((j) => j.rating).reduce((a, j) => a + j.rating, 0) /
    JOB_HISTORY.filter((j) => j.rating).length
  ).toFixed(1);
